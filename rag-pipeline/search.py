...
@click.option("--top-k", "-k", default=None, type=int,
              help="Number of results to return (default: RERANK_TOP_K env var).")
@click.option("--mode", "-m",
              type=click.Choice(["hybrid", "vector", "bm25"]),
              default="hybrid",
              help="Retrieval mode.")
@click.option("--no-rerank", is_flag=True, default=False,
              help="Skip reranker step.")
@click.option("--enhancements", "-e",
              default=None,
              help="Comma-separated query enhancements: hyde,sub_queries,stepback "
                   "(overrides QUERY_ENHANCEMENTS env var).")
def main(query: str, top_k: int | None, mode: str,
         no_rerank: bool, enhancements: str | None) -> None:
