...
OCR_URL="https://${OCR_POD:-UNKNOWN}-8000.proxy.runpod.net"
...
import urllib.request, json, sys
payload = json.dumps({"model": "$model", "input": "test"}).encode()
req = urllib.request.Request("$url/pooling",
      data=payload, headers={"Content-Type": "application/json"}, method="POST")
try:
    resp = json.loads(urllib.request.urlopen(req, timeout=30).read())
    emb = resp["data"][0]["data"][0]
    print(len(emb))
except Exception as e:
    sys.exit(1)
PYEOF
...
MM_DIM=$(detect_pooling_dim "$MM_URL" "nvidia/nemotron-colembed-vl-8b-v2")
[[ -n "$MM_DIM" ]] && echo "  mm-embed dim:   $MM_DIM" || { MM_DIM=4096; echo "  mm-embed dim: 4096 (fallback — verify manually)"; }

OCR_URL_VAL="${OCR_URL:-}"
RERANKER_MODEL_VAL="Qwen/Qwen3-VL-Reranker-2B"

# ── Write .env.runpod ─────────────────────────────────────────────────────────
cat > "$ENV_FILE" <<EOF
# Auto-generated by runpod_wait.sh
# RunPod pods: text=$TEXT_POD mm=$MM_POD reranker=$RERANKER_POD ocr=${OCR_POD:-}
...
# Multimodal embedding — nvidia/nemotron-colembed-vl-8b-v2 (T/I — text-only or image-only)
...
# OCR — PaddlePaddle/PaddleOCR-VL-1.6 (vLLM /v1/chat/completions)
OCR_BASE_URL=${OCR_URL_VAL}/v1
OCR_API_KEY=dummy
...
# Query enhancements disabled (no chat model in this config)
# To enable, add a pod with a chat model and update CHAT_BASE_URL + CHAT_MODEL
QUERY_ENHANCEMENTS=
