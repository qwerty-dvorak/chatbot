...
@click.option("--hypothetical-questions", "-q", is_flag=True, default=False,
              help="Generate hypothetical questions per chunk at index time (requires LLM).")
def main(path: str, strategy: str | None, hypothetical_questions: bool) -> None:
    ...
    try:
        stats = ingest_path(path, strategy=strategy,
    ...
    click.echo(
        f"\nDone — files: {stats['files_processed']}, "
        f"chunks: {stats['chunks_created']}, "
        f"embeddings: {stats['embeddings_indexed']}"
    )
