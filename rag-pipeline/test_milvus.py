...
def main() -> int:
    ...
    try:
        ...
        ok &= check("MilvusClient connected", True)
    except Exception as exc:
        check("MilvusClient connected", False, str(exc))
        return 1
    ...
    try:
        ...
        ok &= check("Collection created + loaded", False, str(exc))
        return 1
    ...
    try:
        ...
        res = client.insert(COLLECTION, rows)
        ok &= check("Inserted 5 rows", True)
    except Exception as exc:
        ok &= check("Inserted 5 rows", False, str(exc))
    ...
    try:
        ...
        ok &= check("Query by filter", False, str(exc))
    ...
    try:
        ...
        ok &= check("Drop collection", False, str(exc))

    # ── Summary ───────────────────────────────────────────────────────────────
    print()
    if ok:
        print(f"  {PASS}  All checks passed — Milvus stack (etcd + minio + milvus) is healthy.")
        return 0
    else:
