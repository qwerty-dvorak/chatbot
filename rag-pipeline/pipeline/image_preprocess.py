...
def preprocess_images(images: list[bytes], max_height: int | None = None) -> list[PreparedImage]:
    ...
    for source_index, raw in enumerate(images):
        ...
        with Image.open(io.BytesIO(raw)) as opened:
            ...
            segments = [
                image.crop((0, top, image.width, min(top + limit, image.height)))
                for top in range(0, image.height, limit)
            ]
            if len(segments) > 1:
            ...
            for segment in segments:
                ...
                prepared.append(PreparedImage(
                    data=output.getvalue(), source_index=source_index,
                    derived_index=derived_index, width=segment.width, height=segment.height,
