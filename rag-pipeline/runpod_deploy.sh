...
TEXT_TPL=$(create_template "rag-text-embed-$TS" \
  ...
  --docker-start-cmd "-c,exec python3 -m vllm.entrypoints.openai.api_server --model nvidia/llama-embed-nemotron-8b --trust-remote-code --port 8000 --gpu-memory-utilization 0.90 --max-model-len 32768" \
  --env "$HF_ENV_JSON" \
  --ports "8000/http" \
...
MM_TPL=$(create_template "rag-mm-embed-$TS" \
  --image "vllm/vllm-openai:latest" \
  --docker-entrypoint "/bin/bash" \
  --docker-start-cmd "-c,exec python3 -m vllm.entrypoints.openai.api_server --model nvidia/nemotron-colembed-vl-8b-v2 --trust-remote-code --port 8000 --gpu-memory-utilization 0.90 --runner pooling" \
  --env "$HF_ENV_JSON" \
  --ports "8000/http" \
  --container-disk-in-gb 50)
...
RERANKER_TPL=$(create_template "rag-reranker-$TS" \
  --image "vllm/vllm-openai:latest" \
  --docker-entrypoint "/bin/bash" \
  --docker-start-cmd '-c,exec python3 -m vllm.entrypoints.openai.api_server --model Qwen/Qwen3-VL-Reranker-2B --trust-remote-code --port 8000 --gpu-memory-utilization 0.90 --hf-overrides "$HF_OVERRIDES"' \
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. OCR — PaddlePaddle/PaddleOCR-VL-1.6 (vLLM /v1/chat/completions)
#    Lowest-cost GPU that can run a 4B VL model: NVIDIA RTX A5000 (24GB, $0.16/hr)
# ─────────────────────────────────────────────────────────────────────────────
OCR_GPU_ID="${OCR_GPU_ID:-NVIDIA RTX A5000}"

echo "Creating OCR template..."
OCR_TPL=$(create_template "rag-ocr-$TS" \
  --image "vllm/vllm-openai:latest" \
  --docker-entrypoint "/bin/bash" \
...
echo "  template: $OCR_TPL"

echo "Creating OCR pod (RTX A5000 — cheapest GPU for 4B VL model)..."
OCR_POD=$(create_pod "rag-ocr" \
  --template-id "$OCR_TPL" \
  --gpu-id "$OCR_GPU_ID" \
  --cloud-type "$CLOUD_TYPE" \
  --container-disk-in-gb 30)
...
echo ""
echo "4 RunPod pods provisioning + local Milvus running. IDs saved to $STATE_FILE"
echo ""
echo "  milvus         (local)      localhost:19530"
echo "  rag-text-embed ($GPU_ID) $TEXT_POD"
echo "  rag-mm-embed   ($GPU_ID) $MM_POD"
echo "  rag-reranker   ($GPU_ID) $RERANKER_POD"
echo "  rag-ocr        ($OCR_GPU_ID) $OCR_POD"
