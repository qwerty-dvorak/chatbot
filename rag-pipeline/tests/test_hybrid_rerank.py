...
Tests:
  ...
  2. Reranker scores overwrite RRF fused scores (method="reranked")
   3. hybrid_search() returns full pool (>= retrieval_top_k, up to 2x)
...
from pipeline.bm25 import build_index as build_bm25_index
...
from pipeline.retrieve import bm25_search, hybrid_search, vector_search
...
def embed_query(text: str) -> list[float]:
    tmp = Chunk(id=uuid.uuid4().hex, source_path="__query__", text=text, chunk_type=ChunkType.TEXT)
...
def run():  # noqa: C901, PLR0912, PLR0915
    ...
    for fpath in TOPIC_FILES:
        ...
        doc = RawDocument(path=fpath, content_type=ContentType.TEXT, text=text, metadata={"filename": fname, "source_path": fpath})
        result = process_document(doc)
        doc_ids.append(result["document_id"])
        print(
            f"    doc_id={result['document_id'][:8]}.. chunks={result['chunks_created']} "
    ...
    all_pg_chunks = pgdb.execute(
        "SELECT id, content, metadata FROM document_chunks ORDER BY chunk_index",
        fetch=True,
    )
    bm25_chunks = []
    ...
    vec_methods = {r.retrieval_method for r in vec_raw}
    check("vector" in vec_methods, "Vector results marked method='vector'")

    bm25_raw = bm25_search(query_text, top_k=retrieval_k)
    check(len(bm25_raw) > 0, f"bm25_search returned {len(bm25_raw)} result(s)")
    bm25_methods = {r.retrieval_method for r in bm25_raw}
    check("bm25" in bm25_methods, "BM25 results marked method='bm25'")
    ...
    check(raw_count >= retrieval_k, f"hybrid pool >= retrieval_top_k ({raw_count} >= {retrieval_k})")
    check(raw_count <= expected_max, f"hybrid pool <= 2x retrieval_top_k ({raw_count} <= {expected_max})")

    distinct_sources = {r.chunk.source_path for r in raw_results}
    ...
    vec_ids = {c.chunk.id for c in vec_raw}
    bm25_ids = {c.chunk.id for c in bm25_raw}
    hybrid_ids = {c.chunk.id for c in raw_results}
    _both_sides = bool(vec_ids & bm25_ids)  # overlap is fine
    _any_unique = bool(hybrid_ids - vec_ids)  # BM25 brought some unique results
    check(len(hybrid_ids) >= len(vec_ids), f"Hybrid pool >= vector pool ({len(hybrid_ids)} >= {len(vec_ids)})")
    ...
    scores_desc = all(results_on[i].score >= results_on[i + 1].score for i in range(len(results_on) - 1))
    ...
    check(hybrid_or_resolved > 0, f"{hybrid_or_resolved}/{len(results_off)} results marked hybrid/query_to_query")
    ...
    vec_methods = {r.retrieval_method for r in vec_results}
    check(vec_methods == {"reranked"}, f"Vector + reranker: all methods='reranked' (got {vec_methods})")
    ...
    check(len(bm25_results_list) > 0, f"BM25 search returned {len(bm25_results_list)} result(s)")
    if bm25_results_list:
        bm25_methods_found = {r.retrieval_method for r in bm25_results_list}
        check(bm25_methods_found == {"reranked"}, f"BM25 + reranker: all methods='reranked' (got {bm25_methods_found})")
    ...
    print("=" * 54)
    if _failures == 0:
        print(" All checks passed — hybrid+rerank workflow validated!")
    else:
