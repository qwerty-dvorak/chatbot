...
class StreamingTest(TestCase):
    ...
    def test_stream_creates_pending_if_none_exists(self) -> None:
        Message.objects.create(chat=self.chat, role=Message.Role.USER, content="Hello", status=Message.Status.COMPLETED)
        response = self.client.get(reverse("chat:stream", args=[self.chat.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_stream_requires_login(self) -> None:
        self.client.logout()
        response = self.client.get(reverse("chat:stream", args=[self.chat.id]))
        self.assertEqual(response.status_code, 302)

    def test_stream_owner_only(self) -> None:
    ...
    def test_stream_content_yields_events(self) -> None:
    ...
    def test_stream_sets_message_status_to_streaming(self) -> None:
        Message.objects.create(chat=self.chat, role=Message.Role.USER, content="Hi", status=Message.Status.COMPLETED)
    ...
    def test_stream_updates_message_content(self) -> None:
        Message.objects.create(chat=self.chat, role=Message.Role.USER, content="Hi", status=Message.Status.COMPLETED)
        pending = Message.objects.create(
            chat=self.chat, role=Message.Role.ASSISTANT, content="", status=Message.Status.PENDING
        )
...
class StreamHandlerTest(TestCase):
    ...
    def test_handle_text_creates_delta_and_returns_event(self) -> None:
    ...
    def test_handle_text_accumulates(self) -> None:
    ...
    def test_channel_markers_split_reasoning_from_final_text(self) -> None:
        events = []
    ...
    def test_split_final_channel_marker_is_not_rendered(self) -> None:
        events = []
    ...
    def test_handle_done_finalizes_message(self) -> None:
    ...
    def test_delta_sequence_increments(self) -> None:
    ...
    def test_handle_reasoning_creates_reasoning_event(self) -> None:
    ...
    def test_handle_reasoning_accumulates(self) -> None:
    ...
    def test_handle_reasoning_then_text(self) -> None:
    ...
    def test_reasoning_stored_in_metadata_on_done(self) -> None:
    ...
    def test_no_reasoning_does_not_set_metadata(self) -> None:
...
class MessageDeltaModelTest(TestCase):
    ...
    def test_delta_sequence_increment(self) -> None:
    ...
    def test_delta_accumulates_content(self) -> None:
        parts = ["Hello", " world", "!"]
    ...
    def test_delta_done_type(self) -> None:
    ...
    def test_multiple_messages_deltas(self) -> None:
        msg2 = Message.objects.create(chat=self.chat, role=Message.Role.ASSISTANT, content="")
        MessageDelta.objects.create(message=self.msg, sequence=0, content="First msg")
        MessageDelta.objects.create(message=msg2, sequence=0, content="Second msg")
        self.assertEqual(MessageDelta.objects.filter(message=self.msg).count(), 1)
